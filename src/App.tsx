      ...
      <div className='flex flex-col gap-10 max-w-[1500px] mx-auto'>
        ...
        <div id='genero'>
          ...
          <div className='flex py-10 px-10'>
            <nav className='w-64 m-0 b-0 list-none flex flex-col gap-5'>
              {todosGeneros.map((g) => (
                <li
                  key={g}
                  onClick={() => setGeneroSelecionado(g)}
                  className={`font-roboto text-xl cursor-pointer transition duration-300 ease-in-out hover:scale-105 p-5 rounded-lg ${generoSelecionado === g ? 'bg-custom-genero ' : ''
                    }`}>
                  {g}
                </li>
              ))}
